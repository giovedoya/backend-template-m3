...
router.get("/", async (req, res, next) => {
  try {
    const reviews = await Review.find().populate("buyerId");
...
// @access  Public
router.get("/reviews/dress/:dressId", async (req, res, next) => {
  try {
    const { dressId } = req.params;
    const reviews = await Review.find({ dressId }).populate("buyerId");
    res.status(200).json(reviews);
  } catch (error) {
    next(error);
  }
...
// @route   GET /reviews/dress/:dressId
// @access  Public
router.get("/reviews/:dressId", async (req, res, next) => {
  try {
    const { dressId } = req.params;
    const reviews = await Review.find({ productId: dressId }).populate(
      "buyerId"
    );
    res.status(200).json(reviews);
  } catch (error) {
...
router.post("/:dressId", isAuthenticated, async (req, res, next) => {
  ...
  if (!rating || !comment) {
    return res
      .status(400)
      .json({ message: "Please provide a rating and comment" });
  }
  try {
...
router.delete("/:reviewId", isAuthenticated, async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(204)
      .json({ message: "the review has been removed successfully" });
  } catch (error) {
    next(error);
  }
});
