const validateDress = (body) =>{
    const { neckline, court, long, color, size, designer, seller, name, description, price, location, image, sold } =
    body;
    if (!neckline || !court || !long || !color || !size || !designer || !seller || !name || !description || !price || !location || !image || !sold){
        return false;
    }
    const allowedProps = ["neckline", "court", "long", "color", "size", "designer", "seller", "name", "description", "price", "location", "image", "sold"];
